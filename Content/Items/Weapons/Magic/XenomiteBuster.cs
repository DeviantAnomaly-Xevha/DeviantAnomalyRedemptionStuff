using DeviantAnomalyRedemptionStuff.Content.Projectiles.Buster.Xenomite;
...
namespace DeviantAnomalyRedemptionStuff.Content.Items.Weapons.Magic
{
    public class XenomiteBuster1 : ModItem
    {
        ...
        public override void HoldItem(Player player)
        {
                ...
                if (Charge > 0f)
                {
                    if (Charge >= 30f)
                    {
                        ...
                        if (Charge >= 30f && Charge < 90f && player.statMana >= Item.mana * 2)
                        {
                            ...
                            int a = Projectile.NewProjectile(Projectile.InheritSource(player), player.Center + AimDirection * 8f, new Vector2(AimDirection.X * Item.shootSpeed, AimDirection.Y * Item.shootSpeed * player.gravDir), Mod.Find<ModProjectile>("XenomiteBusterLv2_Proj").Type, Item.damage * 2, 6, Main.myPlayer);
            ...
            if (Lv3ShotTimer == 9f)
            {
                int a = Projectile.NewProjectile(Projectile.InheritSource(player), player.Center + AimDirection * 8f, new Vector2(AimDirection.X * Item.shootSpeed, AimDirection.Y * Item.shootSpeed * player.gravDir), Mod.Find<ModProjectile>("XenomiteBusterLv4_Proj").Type, Item.damage * 3, 6, Main.myPlayer);
            }
            if (Lv3ShotTimer == 6f)
            {
                int a = Projectile.NewProjectile(Projectile.InheritSource(player), player.Center + AimDirection * 8f, new Vector2(AimDirection.X * Item.shootSpeed, AimDirection.Y * Item.shootSpeed * player.gravDir), Mod.Find<ModProjectile>("XenomiteBusterLv4_Proj").Type, Item.damage * 3, 6, Main.myPlayer);
